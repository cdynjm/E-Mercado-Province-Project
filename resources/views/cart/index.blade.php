                ...
                <div class="card ">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            ...
                            <span class = 'text-danger'>Items from your cart will be removed 5 days after the date of adding.</span>
                        </div>
                        ...
                        <div class="card ">
                            ...
                            <div class="card-body">
                                ...
                                @foreach($carts as $cart)
                                    @if ($grp->SellerID == $cart->SellerID)
                                    <div class="row">
                                        <div class="col-lg-2 col-sm-4">
                                            @if(sizeof($cart->product->images) <= 0)
                                        ...
                                        <div class="col-lg-7 col-sm-8">
                                            ...
                                            <a href = "{{route('search.category', ['searchstring' => $cart->product->producttype->description])}}" class = "link text-primary">{{$cart->product->producttype->description}}</a> / 
                                            <a href = "{{route('search.subcategory', ['searchstring' => $cart->product->kind->description])}}" class = "link text-primary">{{$cart->product->kind->description}}</a></span></div>
                                            <div class = "mt-1"><a href = "#" value = "{{$aes->encrypt($cart->id,auth()->user()->secretkey)}}" class = "badge bg-danger text-white deletecartitem"><i class = 'fa fa-trash'></i> Remove item</a></div>
                                        </div>
                        ...
                        <div class="card ">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div class="card-body">
                                    ...
                                    @foreach($carts as $cart)
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div class="card-action text-warning">
                                                ₱ {{number_format($cart->Amount*$cart->qty, 2, '.', ',')}}
                                            </div>
                                        </div>
                                       <?php
                                            $sum += $cart->Amount*$cart->qty;
                                       ?>
                                    @endforeach
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                    ...
                                    @if (sizeof($cartsgrp) > 0)
                                    <a href = "{{route('order.place')}}" class = "btn btn-warning form-control mt-2">PLACE ORDER</a>
                                    @endif
                                </div>
                            </div>
                        </div>
